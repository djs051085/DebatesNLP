...
D=Debate['Speaker']=='Vice President Joe Biden'
R=Debate['Speaker']=='President Donald J. Trump'
M=Debate['Speaker']=='Chris Wallace'
...
Udf=pd.DataFrame({'key':DVocab,'Dem_Count':DCount}).set_index('key').join(
    pd.DataFrame({'key':RVocab,'Rep_Count':RCount}).set_index('key'),on='key',how='outer').join(
        pd.DataFrame({'key':MVocab,'MCount':MCount}).set_index('key'),on='key',how='outer')
Udf['isStopWord']=Udf.apply(isStopword,axis=1)

Bdf=pd.DataFrame({'key':DBigrams,'DBigram_Count':DBigram_Count}).set_index('key').join(
    pd.DataFrame({'key':RBigrams,'RBigram_Count':RBigram_Count}).set_index('key'),on='key',how='outer').join(
        pd.DataFrame({'key':MBigrams,'MBigram_Count':MBigram_Count}).set_index('key'),on='key',how='outer')

Tdf=pd.DataFrame({'key':DTrigrams,'DTrigram_Count':DTrigram_Count}).set_index('key').join(
    pd.DataFrame({'key':RTrigrams,'RTrigram_Count':RTrigram_Count}).set_index('key'),on='key',how='outer').join(
        pd.DataFrame({'key':MTrigrams,'MTrigram_Count':MTrigram_Count}).set_index('key'),on='key',how='outer')
